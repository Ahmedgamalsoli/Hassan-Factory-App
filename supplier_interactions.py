...
class SupplierInteractions:
    ...
    def supplier_interactions(self, user_role):
        ...
        self.app.payment_entry = ttk.Combobox(left_frame, textvariable=selected_method, 
                                        values=['Cash', 'E_Wallet', 'Bank_account', 'Instapay'], 
                                        state="readonly", width=18)
        self.app.payment_entry.pack(pady=5, padx=10, fill="x")
        self.app.payment_entry.set("Cash")  
        ...
        self.app.supplier_code_cb = ttk.Combobox(right_frame, values=supplier_codes)
        self.app.supplier_code_cb.grid(padx=(10,20), row=1, column=4)

        tk.Label(right_frame, text=self.t("Supplier Name")).grid(padx=(10,20), row=0, column=5)
        self.app.supplier_name_cb = ttk.Combobox(right_frame, values=supplier_names)
        self.app.supplier_name_cb.grid(padx=(10,20), row=1, column=5)

        # Bind combobox events with tree parameter
        self.app.supplier_code_cb.bind("<<ComboboxSelected>>", lambda event: self.CommonInteractions.on_code_selected(
                                      event, self.app.supplier_code_cb, self.app.supplier_name_cb, 
                                      self.supplier_collection, self.purchases_collection, self.supplier_payment_collection, 
                                      "supplier_info.code", tree))
                                      
        self.app.supplier_name_cb.bind("<<ComboboxSelected>>", lambda event: self.CommonInteractions.on_name_selected(
                                      event, self.app.supplier_code_cb, self.app.supplier_name_cb, 
                                      self.supplier_collection, self.purchases_collection, self.supplier_payment_collection, 
                                      "supplier_info.code", tree))
        ...
        search_btn = tk.Button(
            ...
            command=lambda: self.CommonInteractions.on_code_selected(
                None,
                self.app.supplier_code_cb,
                self.app.supplier_name_cb,
        ...
        self.CommonInteractions.update_totals(self.purchases_collection, self.supplier_payment_collection, tree=tree)
    ...
    def add_supplier_payment(self, tree):
        debit = self.app.cash_entry.get().strip()
        ...
        config.report_log(self.app.logs_collection, self.app.user_name, supplier_payment_collection, f"{self.t("Added new record to")}", doc, self.t)
