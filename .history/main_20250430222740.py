...
############################ Utility Functions ########################################
    ...
    def generate_pdf(self, invoice_data):
        ...
        try:
            ...
            width, height = A5  # A5 landscape: 21.0cm x 14.8cm
            c.setFont("Arabic", 14)
            ...
            # ========== إطار وعنوان الفاتورة ==========
            invoice_title = f"فاتورة بيع رقم {invoice_data['Reciept_Number']}"
            
            # رسم الإطار
            frame_width = 4.5*cm
            frame_height = 1.2*cm
            frame_x = width - frame_width - 1.5*cm
            frame_y = height - 3.2*cm
            c.setLineWidth(1)
            c.rect(frame_x, frame_y, frame_width, frame_height)
            
            # كتابة العنوان داخل الإطار
            c.setFont("Arabic", 16)
            title_x = width - 2.2*cm
            title_y = height - 2.8*cm
            c.drawRightString(title_x, title_y, format_arabic(invoice_title))
            ...
            y_position = height - 4.2*cm
            c.setFont("Arabic", 12)
            for line in company_info:
                c.drawRightString(width - 2.2*cm, y_position, format_arabic(line))
                y_position -= 0.8*cm

            # ========== معلومات العميل ==========
            customer_y = height - 6.5*cm
            c.setFont("Arabic", 12)
            customer_fields = [
            ...
            for field, value in customer_fields:
                ...
                c.drawString(1.5*cm, customer_y, text)
                customer_y -= 0.8*cm

            # ========== جدول العناصر ==========
            headers = ["كود الصنف", "العدد", "الوحدة", "السعر", "الكمية", "الإجمالي"]
            col_positions = [
                width - 2.2*cm,    # كود الصنف
                width - 4.5*cm,    # العدد
                width - 6.5*cm,    # الوحدة
                width - 8.5*cm,    # السعر
                width - 11*cm,     # الكمية
                width - 14*cm      # الإجمالي
            ]
            ...
            totals = [
                ...
                ("حساب سابق:", invoice_data.get('Previous_balance', 0)),
                ("إجمالي الفاتورة:", invoice_data['Net_total'] + invoice_data.get('Previous_balance', 0)),
                ("المدفوع:", invoice_data.get('Payed_cash', 0)),
                ("الباقي:", (invoice_data['Net_total'] + invoice_data.get('Previous_balance', 0)) - invoice_data.get('Payed_cash', 0))
            ]
            
            c.setFont("Arabic", 12)
            for label, value in totals:
                text = f"{format_arabic(label)} {format_arabic(f'{value:,.2f}')}"
                c.drawRightString(width - 2.2*cm, totals_y, text)
                totals_y -= 0.8*cm

            # ========== التوقيعات ==========
            c.setFont("Arabic", 10)
            c.drawRightString(width - 2.2*cm, 2.5*cm, format_arabic("ختم وتوقيع المدير: __________"))
            c.drawString(1.5*cm, 2.5*cm, format_arabic("الاستلام: __________"))
