...
############################################ Windows ########################################### 
    ...
    def main_menu(self):
        ...
        buttons = [
            ...
            "command": lambda: self.show_visualizations(self.user_role)},
        ]
        # data = {
    ...
    def create_left_visualization(self, parent):
        try:
            ...
            fig = plt.Figure(figsize=(6, 8), dpi=60)  # Increased height for two charts
            fig.subplots_adjust(hspace=0.4)  # Add vertical spacing
            ...
            ax1.bar(['Customers', 'Suppliers'], 
                [data['customers'], data['suppliers']], 
                color=['#003366', '#009999'])
            ax1.set_title("Customer & Supplier Count")
            ax1.set_ylabel("Count")

            # Second subplot - Summary Table
    ...
    def create_right_visualization(self, parent):
        try:
            data = {
                'sales': self.get_sales_count(),
                'purchases': self.get_purchase_count(),
                'top_client': self.get_top_client()  # Add top_client to data
            }

            # Create figure with 2 vertical subplots
            fig = plt.Figure(figsize=(6, 8), dpi=60)  # Increased height to accommodate both charts
            fig.subplots_adjust(hspace=0.5)  # Add spacing between subplots

            # First subplot (top)
            ax1 = fig.add_subplot(211)  # 2 rows, 1 column, position 1
            ax1.pie([data['sales'], data['purchases']],
            ...
            ax2 = fig.add_subplot(212)  # 2 rows, 1 column, position 2
            if data['top_client']:
                ax2.bar(data['top_client'][0], data['top_client'][1],
                        color='#9467bd')
