...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        # Fetch product data and create instance variable
        products = list(products_col.find())
        self.product_map = {
        ...
        # Create header row
        create_row(header_row, 0, None)
    def update_product_info(self, event=None, row_idx=None):
        """Update unit and price when product is selected"""
        if row_idx is None:
            # Get row index from event widget
            widget = event.widget
        ...
        # Update product name combobox
        self.entries[row_idx][1].set(product_info.get('name', ''))
        
        # Update unit combobox
