...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        col_width = 20  # Character width for all columns
        min_width = 120  # Minimum pixel width for columns
        
        # Create main container frame
        grid_container = tk.Frame(form_frame)
        grid_container.grid(row=2, column=0, columnspan=len(columns)+1, sticky='nsew', pady=(20, 0))

        # Configure uniform column weights
        for i in range(len(columns)):
            grid_container.columnconfigure(i, weight=1, minsize=min_width, uniform='cols')

        # Header Row
        header_row = tk.Frame(grid_container)
        header_row.grid(row=0, column=0, columnspan=len(columns), sticky='nsew')

        # Create header cells
        for col_idx, col in enumerate(columns):
            cell = tk.Label(header_row, 
                        text=col, 
                        anchor='w',
                        relief='ridge',
                        bg='#f0f0f0',
            ...
            header_row.columnconfigure(col_idx, weight=1, minsize=min_width, uniform='cols')
        ...
        # Grid layout
        canvas.grid(row=1, column=0, columnspan=len(columns), sticky='nsew')
        scrollbar.grid(row=1, column=len(columns), sticky='ns')

        # Configure grid weights
        grid_container.rowconfigure(1, weight=1)
        for i in range(len(columns)):
            self.rows_frame.columnconfigure(i, weight=1, minsize=min_width, uniform='cols')

        # Entry management
        self.entries = []

        def create_data_row(row_num):
            ...
            for col_idx in range(len(columns)):
                ...
                entry.grid(row=0, column=col_idx, sticky='nsew', padx=0, pady=0)
                row_frame.columnconfigure(col_idx, weight=1, minsize=min_width, uniform='cols')
        ...
        def add_three_rows():
            ...
            for i in range(3):
                self.entries.append(create_data_row(current_row + i))
