...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        customer_frame.grid(row=0, column=0, columnspan=2, sticky='ew', pady=5)  # Make it stretch
        ...
        self.customer_cb = ttk.Combobox(customer_frame,
                                         ...
                                         width=28,
                                         state='readonly')
        self.customer_cb.grid(row=0, column=1, sticky='ew', padx=(5, 0))  # Make it stretch

        # Configure grid weights for the customer frame
        ...
        col_width = 20  # Consistent width for all columns, adjusted for better fit

        # Header Frame
        header_frame = tk.Frame(form_frame, bg='#f0f0f0')
        header_frame.grid(row=2, column=0, columnspan=2, sticky='ew', pady=(20, 0))  # Span 2 columns

        # Create headers with consistent column configuration
        for idx, col in enumerate(columns):
            label = tk.Label(header_frame, text=col, width=col_width, relief='ridge',
                             bg='#f0f0f0', padx=7, anchor='w')
            label.grid(row=0, column=idx, sticky='ew')  # Use 'ew' to stretch headers
            header_frame.columnconfigure(idx, weight=1)  # Ensure columns can expand
        ...
        # Grid layout
        canvas.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=(0, 10))  # Span 2 columns
        scrollbar.grid(row=3, column=2, sticky="ns", pady=(0, 10))

        # Configure grid weights for the form frame
        form_frame.grid_rowconfigure(3, weight=1)
        form_frame.columnconfigure(0, weight=1)  # Important: Make the first column expandable
        form_frame.columnconfigure(1, weight=1)  # Make the second column expandable too, if needed.
        ...
        def add_three_rows():
            ...
            for i in range(3):
                ...
                row_frame.grid(row=current_row_count + i, column=0, sticky='ew')  # Removed лишний column=0
                ...
                for col_idx in range(len(columns)):
                    ...
                    row_frame.columnconfigure(col_idx, weight=1)  # Make columns expandable
        ...
        button_frame.grid(row=4, column=0, columnspan=2, pady=10, sticky='ew')  # Span 2 columns
...
############################ Utility Functions ########################################
    ...
    def get_fields_by_name(self, collection_name):
        ...
        elif collection_name == "Sales_Record":
