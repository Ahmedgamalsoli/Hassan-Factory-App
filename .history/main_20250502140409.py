...
############################################ Windows ########################################### 
    ...
    def calculate_totals(self, row_idx):
        try:
            ...
            numbering = float(self.entries[row_idx][3].get() or 0)  # numbering @ index 3
            qty = float(self.entries[row_idx][4].get() or 0)        # QTY @ index 4
            unit_price = float(self.entries[row_idx][8].get() or 0)  # Unit_Price @ index 8
            discount_type = self.entries[row_idx][5].get()           # Discount Type @ index 5
            discount_value = float(self.entries[row_idx][6].get() or 0)  # Discount Value @ index 6
