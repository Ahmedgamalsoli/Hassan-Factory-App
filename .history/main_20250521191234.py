    ...
    def get_top_client(self):
        try:
            pipeline = [
                ...
                # Sort by creditNumeric (descending) and keep all fields
                {"$sort": {"creditNumeric": -1}},
                # Keep only the top client
                {"$limit": 1},
                # Explicitly include the Name and creditNumeric fields
                {"$project": {"Name": 1, "creditNumeric": 1, "_id": 0}}
            ]
            result = list(self.sales_collection.aggregate(pipeline))
            
            if result:
                return (result[0]["Name"], result[0]["creditNumeric"])
