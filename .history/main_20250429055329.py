    ...
    def new_sales_invoice(self, user_role):
        ...
        try:
            ...
            for p in products:
                ...
                units_list = p.get('Units', [])  # get Units array

                # Make sure units_list is a list
                if not isinstance(units_list, list):
                    units_list = []

                self.product_map[code] = {
                    'name': name,
                    'units': units_list,  # save the list of units directly
            ...
            # units now is the set of all available unit strings across all products
            units = list({unit for v in self.product_map.values() for unit in v['units_list']})
