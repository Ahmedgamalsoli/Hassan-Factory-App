...
############################################ Windows ########################################### 
    ...
    def show_visualizations(self):
        try:
            ...
            vis_window.state("zoomed")  # Maximize the window
            ...
            fig.suptitle("Business Performance Dashboard", fontsize=18, y=0.95)
            ...
            # Chart 1: Customers vs Suppliers (Bar Chart)
            ax1.bar(['Customers', 'Suppliers'], 
                    [data['customers'], data['suppliers']], 
                    color=['#1f77b4', '#ff7f0e'])
            ax1.set_title("Customer & Supplier Count", pad=20)
            ax1.set_ylabel("Count", labelpad=10)
            
            # Chart 2: Sales/Purchases Ratio (Pie Chart)
            ax2.pie([data['sales'], data['purchases']], 
                    labels=['Sales', 'Purchases'],
                    autopct='%1.1f%%',
                    colors=['#2ca02c', '#d62728'],
                    startangle=90)
            ax2.set_title("Sales vs Purchases Ratio", pad=20)

            # Chart 3: Top Client (Bar Chart)
            if data['top_client']:
                ax3.bar(data['top_client'][0], data['top_client'][1], 
                        color='#9467bd')
                ax3.set_title("Top Performing Client", pad=20)
                ax3.set_ylabel("Sales Amount", labelpad=10)
                ax3.ticklabel_format(style='plain', axis='y')  # Disable scientific notation
            ...
            # Add close button
            close_btn = tk.Button(vis_window, text="Close", 
            ...
            tk.messagebox.showerror("Error", "Failed to generate visualizations")
