...
############################ Utility Functions ########################################
    ...
    def update_search(self,event, collection):
        ...
        # Auto-open dropdown only if there are results
        if filtered_customers:
            self.customer_cb.event_generate('<Down>')
        else:
            self.customer_cb.event_generate('<Up>')  # Close if no results
        # Cancel previous timer if exists
        if hasattr(self, '_timer'):
            self._timer.cancel()
        
        # Schedule filtering after 200ms
        self._timer = Timer(0.2, do_filter)
        self._timer.start()
