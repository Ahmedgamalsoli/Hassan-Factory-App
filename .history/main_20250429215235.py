...
############################ Utility Functions ########################################
    ...
    def save_invoice(self, sales_col, customers_col):
        try:
            # Get customer data
            customer = customers_col.find_one({"Customer_code": self.customer_var.get().strip()})
            if not customer:
            ...
            for row in self.entries:
                if not row[0].get() and not row[1].get():  # Skip empty rows
                    continue
            ...
            invoice = {
                "Recital_number": self.generate_invoice_number(),
                ...
                "Amount_paid": 0.0,  # Should be collected from UI
                "Remaining_balance": total_amount + float(customer.get('Previous_balance', 0))
            }

            # Insert into database
            result = sales_col.insert_one(invoice)
            ...
            messagebox.showinfo("Success", f"Invoice {invoice['Recital_number']} saved successfully!")
