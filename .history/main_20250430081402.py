...
############################ Utility Functions ########################################
    ...
    def generate_invoice_number(self):
        ...
        try:
            ...
            sales_col = self.get_collection_by_name('Sales')
            sales_col.update_many(
                {"Reciept_Number": {"$regex": "INV-"}},
                [{"$set": {"Reciept_Number": {"$substrBytes": ["$Reciept_Number", 4, 4]}}}]
)
            ...
            if last_invoice:
                ...
                # 4. معالجة التنسيقات (نصوص أو أعداد)
                # if isinstance(receipt_number, str):
                #     try:
                #         # تجاهل أي أحرف غير رقمية وتحويل الباقي إلى عدد
                #         cleaned_number = ''.join(filter(str.isdigit, receipt_number))
                #         last_number = int(cleaned_number) if cleaned_number else 0
                #     except ValueError:
                #         last_number = 0
            ...
            # 5. توليد الرقم الجديد مع أصفار أمامية
            new_number = last_number + 1
            return f"{new_number:04d}"  # مثال: 29 → "0030"
