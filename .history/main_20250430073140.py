...
############################ Utility Functions ########################################
    ...
    def generate_invoice_number(self):
        ...
        try:
            ...
            last_number = 0
            if last_invoice:
                receipt_number = last_invoice.get("Reciept_Number")
                
                # معالجة التنسيقات القديمة (INV-xxxx)
                if isinstance(receipt_number, str) and receipt_number.startswith("INV-"):
                    try:
                        last_number = int(receipt_number.split("-")[-1])
                    except (ValueError, IndexError):
                        last_number = 0
                # معالجة الأرقام المخزنة كنصوص أو أعداد
            ...
            return str(new_number)  # إرجاع الرقم كنص
