...
############################ Utility Functions ########################################
    ...
    def generate_invoice_number(self):
        ...
        try:
            ...
            last_number = 0
            if last_invoice:
                recital_number = last_invoice.get("Reciept_Number")
                # Validate the format of Recital_number
                if (
                    recital_number 
                    and isinstance(recital_number, str) 
                    and "-" in recital_number
                ):
                    try:
                        # Extract the numeric part and convert to integer
                        last_number = int(recital_number.split("-")[-1])
                    except (ValueError, IndexError):
                        # Fallback if the suffix is invalid
                        last_number = 0
    ...
    def save_invoice(self, sales_col, customers_col):
        ...
        try:
            ...
            customer = customers_col.find_one({"Name": customer_name})
            if not customer:
                messagebox.showerror("خطأ", "العميل غير مسجل!")  # Fixed typo: shoreror → showerror
                return
    ...
    def generate_pdf(self, invoice_data):
        ...
        try:
            ...
            # تنسيق الخطوط والعناصر
            c.setFont("Helvetica-Bold", 14)
            c.drawString(50, height-50, "مصنع حسن سليم للمنتجات البلاستيكية")
            
            # معلومات الفاتورة الأساسية
            c.setFont("Helvetica", 10)
            c.drawString(50, height-80, f"رقم الفاتورة: {invoice_data['Reciept_Number']}")
