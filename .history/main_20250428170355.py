...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        header_frame.grid(row=2, column=0, columnspan=3, sticky='ew')
        for idx, col in enumerate(columns):
            tk.Label(header_frame, text=col, width=15, relief='ridge').grid(row=0, column=idx)

        # Container for data rows
        self.rows_frame = tk.Frame(form_frame)
        self.rows_frame.grid(row=3, column=0, columnspan=3, pady=5)
        ...
        def add_row():
            ...
            for col_idx in range(len(columns)):
                entry = tk.Entry(row_frame, width=15, relief='sunken')
                entry.grid(row=0, column=col_idx, padx=1, pady=1)
                row_entries.append(entry)
            self.entries.append(row_entries)

        # Add Product Button
        tk.Button(form_frame, text="Add Row", command=add_row).grid(row=4, column=0, pady=5)

        # Save Invoice Button
        tk.Button(form_frame, text="Save Invoice", 
                command=lambda: self.save_invoice(sales_col, customers_col)).grid(row=4, column=1, pady=5)
############################ Main Functions ########################################
