...
############################################ Windows ########################################### 
    ...
    def calculate_totals(self, row_idx):
        try:
            # Get basic values
            qty = float(self.entries[row_idx][4].get() or 0)        # QTY index
            numbering = float(self.entries[row_idx][3].get() or 0)  # numbering index
            unit_price = float(self.entries[row_idx][6].get() or 0) # Unit_Price index
            
            # Get discount values
            discount_type = self.entries[row_idx][8].get()          # Discount Type index
            discount_value = float(self.entries[row_idx][9].get() or 0)  # Discount Value index
            ...
            if discount_type == "Percentage":
                if discount_value < 0 or discount_value > 100:
                    raise ValueError("Invalid percentage (0-100)")
                discount = total_price * (discount_value / 100)
            else:  # Value discount
                discount = min(discount_value, total_price)
                
            final_price = max(total_price - discount, 0)  # Prevent negative prices
            ...
            self.entries[row_idx][7].config(state='normal')
            self.entries[row_idx][7].delete(0, tk.END)
            self.entries[row_idx][7].insert(0, f"{final_price:.2f}")
            self.entries[row_idx][7].config(state='readonly')

        except ValueError as e:
            # Handle specific error cases
            if "Invalid percentage" in str(e):
                messagebox.showerror("Discount Error", "Percentage must be between 0-100")
                self.entries[row_idx][9].delete(0, tk.END)
                self.entries[row_idx][9].insert(0, "0")
