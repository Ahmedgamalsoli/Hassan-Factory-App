...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        # Customer Selection
        tk.Label(form_frame, text="Customer:", font=("Arial", 15, "bold")).grid(row=0, column=0, sticky='w', pady=5)
        self.customer_var = tk.StringVar()
        self.customer_cb = ttk.Combobox(form_frame, textvariable=self.customer_var, width=30)
        self.customer_cb.grid(row=0, column=0, sticky='w', padx=5, pady=5)
        ...
        header_frame.grid(row=2, column=0, columnspan=len(columns), sticky='ew', pady=(20, 0))
        
        # Create headers
        for idx, col in enumerate(columns):
            tk.Label(header_frame, text=col, width=18, relief='ridge', bg='#f0f0f0').grid(
                row=0, column=idx, sticky='ew', padx=1, pady=1)
        ...
        # Grid layout
        canvas.grid(row=3, column=0, columnspan=len(columns)-1, sticky="nsew", pady=(0, 10))
        ...
        def add_three_rows():
            ...
            for i in range(3):
                ...
                for col_idx in range(len(columns)):
                    entry = tk.Entry(row_frame, width=18, relief='sunken', bg='white' if current_row_count+i % 2 == 0 else '#f8f8f8')
                    entry.grid(row=0, column=col_idx, padx=1, pady=1, ipady=3)
