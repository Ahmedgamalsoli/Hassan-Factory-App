...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        self.customer_cb.bind('<KeyRelease>', lambda event: self.update_search(event, customers_col),add='+')
...
############################ Utility Functions ########################################
    ...
    def update_search(self, event, collection):
        ...
        # Use cached customer names for faster filtering
        if not hasattr(self, 'all_customers'):
            self.all_customers = [cust['Name'] for cust in collection.find()]
        
        # Filter using regex in memory (no database query)
        if search_term:
            pattern = re.compile(f'^{re.escape(search_term)}', re.IGNORECASE)
            filtered = [name for name in self.all_customers if pattern.match(name)]
        else:
        ...
        if filtered:
            self.customer_cb.after(50, lambda: self.customer_cb.event_generate('<Down>'))
        else:
            self.customer_cb.event_generate('<Up>')
        
        # Allow continued typing by returning "break" to prevent default behavior
        return "break"
