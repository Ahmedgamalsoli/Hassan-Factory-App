...
############################################ Windows ########################################### 
    ...
    def show_visualizations(self):
        ...
        ax1.bar(['Customers', 'Suppliers'], [data['customers'], data['suppliers']], 
               color=['#003366', '#009999'])
        ax1.set_title("Customer & Supplier Count")
        ax1.set_ylabel("Count")

        # Chart 2: Sales vs Purchases
        ax2.pie([data['sales'], data['purchases']], 
              labels=['Sales', 'Purchases'], 
              autopct='%1.1f%%', 
              colors=['#4CAF50', '#FF5252'])
        ax2.set_title("Sales vs Purchases Ratio")

        # Chart 3: Top Client
        if data['top_client']:
            ax3.bar(data['top_client'][0], data['top_client'][1], color='#9C27B0')
            ax3.set_title("Top Performing Client")
    ...
    # Database query methods (implement with your actual DB connection)
