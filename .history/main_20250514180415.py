...
############################################ Windows ########################################### 
    ...
    def new_Purchase_invoice(self, user_role):
        ...
        self.customer_name_var = tk.StringVar()
        self.customer_name_cb = ttk.Combobox(Supplier_frame, textvariable=self.customer_name_var, values=sorted(all_suppliers))
        self.customer_name_cb.grid(row=0, column=1, padx=5, sticky='ew')

        # Customer Code Combobox
        tk.Label(Supplier_frame, text=self.t("Supplier Code"), font=("Arial", 12, "bold")).grid(row=0, column=2, sticky='w')
        self.customer_code_var = tk.StringVar()
        self.customer_code_cb = ttk.Combobox(Supplier_frame, textvariable=self.customer_code_var, values=sorted(all_codes))
        self.customer_code_cb.grid(row=0, column=3, padx=5, sticky='ew')
        ...
        self.customer_name_cb.bind('<KeyRelease>', lambda e: [
            self.filter_combobox(e, all_suppliers, self.customer_name_cb),
            sync_from_name()
        ])
        ...
        # Load product data
        try:
            products = list(materials_col.find())
            all_units = set()
            product_names = []
            product_codes = []
        ...
        tk.Button(button_frame, text="➕ Add 3 More Rows", command=add_three_rows,
                bg='#4CAF50', fg='white').grid(row=0, column=0, padx=5, sticky='w')
        tk.Button(button_frame, text="💾 Save Invoice", 
                command=lambda: self.save_invoice(purchases_col, suppliers_col,materials_col),
                bg='#2196F3', fg='white').grid(row=0, column=1, padx=5, sticky='e')
    ...
    def update_material_info(self, row_idx, source):
        ...
        try:
            if source == "code":
                ...
                material_name = product_info.get('name', '')
            else:
