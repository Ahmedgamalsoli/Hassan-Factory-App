...
############################ Utility Functions ########################################
    ...
    def upload_pdf_to_cloudinary(file_path_param):
        try:
            # Step 1: Upload the PDF (NO flags here)
            response = cloudinary.uploader.upload(
                file_path_param,
                resource_type="raw",  # Required for PDFs
                public_id="my_pdf",   # Optional: Set a custom public_id
            )
            
            # Step 2: Generate the URL with transformations
            public_id = response["public_id"]
            pdf_url = cloudinary.CloudinaryImage(public_id).build_url(
                resource_type="raw",
                flags="inline",  # Add transformations here
                type="upload"
            )
