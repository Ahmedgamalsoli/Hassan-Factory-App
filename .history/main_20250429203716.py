    ...
    def new_sales_invoice(self, user_role):
        ...
        try:
            ...
            for p in products:
                ...
                # Ensure units_list is a list
                if not isinstance(units_list, list):
                    units_list = []
                
                # Extract valid unit names from units_list (handling both dict and string)
                unit_names = []
                for unit in units_list:
                    if isinstance(unit, dict):
                        # Assuming unit name is stored under 'unit_name' key
                    ...
                    if unit_name:  # Only add non-empty names
                        unit_names.append(unit_name)
                
                # Get price (assuming product-level Unit_Price is correct)
            ...
            # Generate lists for product codes, names, and units
            product_codes = list(self.product_map.keys())
            product_names = list({v['name'] for v in self.product_map.values()})
            units = list({unit for v in self.product_map.values() for unit in v['units']})
