...
############################ Init ########################################################
    ...
    def __init__(self, root):
        ...
        self.units_combobox = ttk.Combobox(self.root)  # <-- THIS WAS MISSING
        self.units_combobox.pack(pady=10)  # Adjust layout as needed
...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        try:
            ...
            self.product_map = {}  # Reset product mappings
            self.name_to_code = {}
            all_units = set()  # Track all unique units (if needed globally)
            ...
            for p in products:
                ...
                # Extract valid unit names
                unit_names = []
                for unit in units_list:
                    if isinstance(unit, dict):
                        # Adjust key based on your data (e.g., 'name' instead of 'unit_name')
                        unit_name = str(unit.get('name', '')).strip()  # Change key here if needed
                    ...
                    if unit_name:
                        ...
                        all_units.add(unit_name)  # Add to global set

                # Update product mappings with product-specific units
                self.product_map[code] = {
                    'name': name,
                    'units': unit_names,  # Units specific to this product
                    'price': float(p.get('Unit_Price', 0))
                }
                self.name_to_code[name] = code

            # Use `all_units` for a global dropdown OR bind dynamically per product
            self.units_combobox['values'] = list(all_units)  # Example for Tkinter Combobox
    ...
    def get_units_by_code(self, code):
        code = code.strip()
        product = self.product_map.get(code)
        if product:
            return product.get('units', [])
        return []
