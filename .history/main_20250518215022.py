...
############################################ Windows ########################################### 
    ...
    def create_left_visualization(self, parent):
        try:
            ...
            ax.bar(['Customers', 'Suppliers'], 
                   [data['customers'], data['suppliers']], 
                   color=['#003366', '#009999'])
            ax.set_title("Customer & Supplier Count")
    ...
    def create_right_visualization(self, parent):
        try:
            ...
            fig = plt.Figure(figsize=(6, 4), dpi=100)
            ax = fig.add_subplot(111)
            ax.pie([data['sales'], data['purchases']],
                   labels=['Sales', 'Purchases'],
                   autopct='%1.1f%%',
                   colors=['#4CAF50', '#FF5252'])
            ax.set_title("Sales vs Purchases Ratio")
    ...
    def create_main_buttons(self, parent):
        ...
        try:
            for index, btn_info in enumerate(buttons):
                ...
                btn = tk.Button(btn_frame, image=photo_img, 
                              command=btn_info["command"], bd=0)
                btn.image = photo_img
                btn.pack()

                lbl = tk.Label(btn_frame, text=btn_info["text"], 
            ...
            for btn_info in buttons:
                tk.Button(parent, text=btn_info["text"], 
                         command=btn_info["command"]).pack()

    def show_detailed_reports(self):
        # Implement detailed reports window
        pass

    # Database methods (implement with actual queries)
