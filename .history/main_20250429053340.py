    ...
    def new_sales_invoice(self, user_role):
        ...
        try:
            ...
            for p in products:
                ...
                # Fetch the Units array properly
                units = p.get('Units', [])
                if isinstance(units, list):
                    unit_str = ', '.join(units)  # Join all units into a single string separated by commas
                else:
                    unit_str = ''  # fallback in case 'Units' is not a list
