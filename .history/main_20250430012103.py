...
def generate_pdf(self, invoice_data):
    ...
    try:
        ...
        import os
        from bidi.algorithm import get_display  # معالجة النصوص العربية
        import arabic_reshaper  # إعادة تشكيل الحروف العربية
        ...
        # العنوان الرئيسي (معالجة النص)
        title = format_arabic("مصنع حسن سليم للمنتجات البلاستيكية")
        c.drawRightString(width - 2*cm, height - 2*cm, title)

        # معلومات الفاتورة
        info_y = height - 4*cm
        invoice_number = format_arabic(f"رقم الفاتورة: {invoice_data['Reciept_Number']}")
        c.drawRightString(width - 2*cm, info_y, invoice_number)
        
        date = format_arabic(f"التاريخ: {invoice_data['Date']}")
        c.drawRightString(2*cm, info_y, date)
