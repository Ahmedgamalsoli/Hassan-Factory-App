...
############################################ Windows ########################################### 
    ...
    def new_sales_invoice(self, user_role):
        ...
        col_width = 29  # Consistent width for all columns
        
        # Header Frame
        header_frame = tk.Frame(form_frame, bg='#f0f0f0')
        header_frame.grid(row=2, column=0, columnspan=len(columns), sticky='nsew', pady=(20, 0))
        
        # Create headers with consistent column configuration
        for idx, col in enumerate(columns):
            tk.Label(header_frame, text=col, width=col_width-10, relief='ridge',
                    bg='#f0f0f0', padx=7, anchor='w').grid(row=0, column=idx, sticky='ew')
